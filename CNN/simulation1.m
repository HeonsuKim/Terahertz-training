...
%a(:,4)=a(:,3);
%a(:,5)=pulse_restore_td(length_5*2,refractive_index,extinction_coeff,a(:,4),time(:,1));
...
for i=1:3  %5
    ...
    %plot(generate_pulse(:,1),generate_pulse(:,i+1))
    else
    aa=reflection_generator(THz_waveform_denoised(:,repeat),a(:,i),time(:,1),resolution,delta(i,1));
    generate_pulse(:,i+1)=-aa(:,2);
    generate_pulse(:,1)=aa(:,1);
...
%simulation_result(:,2)=spline_ref(:,2)+generate_pulse(:,2)+generate_pulse(:,3)+generate_pulse(:,4)+generate_pulse(:,5)+generate_pulse(:,6);
...
% % % %plot(TD(:,1),TD(:,2))
% % % %plot(TD(:,1),TD(:,3))
% % % plot(simulation_result(:,1),simulation_result(:,2))
% % % 
...
%plot(time(:,1),reduced_wave(:,1))
% % % 
% % % xlswrite('wave1.xlsx',reduced_wave)

% % % figure(2)
% % % hold on
% % % plot(spline_ref(:,1),spline_ref(:,2))
% % % plot(generate_pulse(:,1),generate_pulse(:,2))
% % % plot(generate_pulse(:,1),generate_pulse(:,3))
% % % plot(generate_pulse(:,1),generate_pulse(:,4))

% nana=(21.1583+23.9929)*rand(1995,1)-23.9929;
% spline_temp(:,2)=nana;
% spline_ref=cubic_spline(spline_temp,resolution);
% simulation_result_temp(:,1)=simulation_result(:,2)+spline_ref(:,2);
% 
% 
% figure(3)
% hold on
% %plot(TD(:,1),TD(:,2))
% plot(TD(:,1),TD(:,3))
% plot(simulation_result(:,1),simulation_result_temp(:,1))
end
